...
function App() {
  ...
  // Fórmula: Ángulo del destino - Ángulo hacia donde miro con el teléfono
  const rotacionFlecha = rumbo !== null ? (rumbo - brujulaTelefono) : 0;

  return (
    <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif', textAlign: 'center', backgroundColor: '#121212', color: '#e0e0e0', minHeight: '100vh' }}>
      <h1 style={{ fontSize: '26px', color: '#4CAF50', marginBottom: '5px' }}>Retorno Seguro 🌲</h1>
      <p style={{ opacity: 0.7, fontSize: '14px' }}>Brújula táctica offline para senderismo</p>

      {errorGps && <p style={{ color: '#ff6b6b', backgroundColor: '#2c1a1a', padding: '10px', borderRadius: '5px' }}>⚠️ {errorGps}</p>}

      {!puntoPartida ? (
        <div style={{ marginTop: '60px' }}>
          <p style={{ fontSize: '16px', margin: '20px' }}>👉 Estás en la base del cerro. Registra tu posición actual antes de perder la señal.</p>
          <button 
            onClick={registrarPuntoPartida} 
            style={{ padding: '18px 35px', fontSize: '18px', fontWeight: 'bold', backgroundColor: '#4CAF50', color: 'white', border: 'none', borderRadius: '30px', boxShadow: '0 4px 15px rgba(76,175,80,0.3)', cursor: 'pointer' }}
          >
        ...
        <div style={{ marginTop: '20px' }}>
          ...
          <div style={{ margin: '30px auto', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            ...
            <div style={{ 
              ...
              {/* Flecha SVG que rota dinámicamente con CSS */}
              <svg 
                style={{ 
                  width: '100px', 
                  height: '100px', 
                  transform: `rotate(${rotacionFlecha}deg)`, 
                  transition: 'transform 0.2s ease-out', // Suaviza el movimiento al girar
                  fill: distancia && distancia < 0.1 ? '#4CAF50' : '#2196F3' // Cambia a verde si estás muy cerca
                }} 
                viewBox="0 0 24 24"
              >
              ...
              {/* Indicador de Norte de la pantalla (opcional) */}
              <span style={{ position: 'absolute', top: '10px', fontSize: '12px', fontWeight: 'bold', color: '#666' }}>N</span>
            </div>

            {/* Datos de telemetría */}
            <div style={{ marginTop: '20px', padding: '15px 30px', borderRadius: '15px', backgroundColor: '#1c1c1c', border: '1px solid #333', inlineSize: '280px' }}>
              <p style={{ margin: '5px 0', fontSize: '14px', color: '#aaa' }}>DISTANCIA AL ORIGEN</p>
          ...
          <button 
            onClick={() => { if(window.confirm("¿Seguro que quieres borrar la ruta actual?")) { localStorage.clear(); setPuntoPartida(null); setDistancia(null); } }}
            style={{ backgroundColor: 'transparent', color: '#ff6b6b', border: '1px solid #ff6b6b', padding: '8px 15px', borderRadius: '20px', cursor: 'pointer', fontSize: '12px', marginTop: '20px' }}
          >
            Reiniciar Ruta
          </button>
        </div>
